...
static void process_marithmetic_command(conn *c, token_t *tokens, const size_t ntokens) {
    ...
    enum arith incr = increment; // default mode is to increment.
    bool locked = false;
    uint32_t hv = 0;
...
static void process_arithmetic_command(conn *c, token_t *tokens, const size_t ntokens, const enum arith incr) {
...
void process_command_ascii(conn *c, char *command) {
    ...
    if (first == 'm' && tokens[COMMAND_TOKEN].length == 2) {
        ...
        if (strcmp(tokens[COMMAND_TOKEN].value, "delete") == 0) {
            ...
            WANT_TOKENS_OR(ntokens, 4, 5);
            process_arithmetic_command(c, tokens, ntokens, divide);
        ...
        if (strcmp(tokens[COMMAND_TOKEN].value, "mult") == 0) {
            ...
            WANT_TOKENS_OR(ntokens, 4, 5);
            process_arithmetic_command(c, tokens, ntokens, multiplication);
        }else{
            out_string(c, "ERROR");
        }
    }  else if (first == 't') {
